...
class Calibrator:
    ...
    def __init__(self, keep_data: bool = True, datapath='eye_commander/temp',  cameraid: int = 0):

        self.TEMP_PATH = resource_path(datapath)
        self.camera = image_capture.Camera(source=cameraid)
            ...
            for frame in frames:
                ...
                if eyes:
                    ...
                    cv2.imwrite(os.path.join(
                        class_path, f'{key}{count}.jpg'), left)

                    cv2.imwrite(os.path.join(
                        class_path, f'{key}{count+1}.jpg'), right)
