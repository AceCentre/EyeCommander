...
// import { vJoy, vJoyDevice } from "ace-vjoy";
...
class KeyboardLoggingEmulator extends KeyboardEmulator {
  constructor() {
    super();
  }
...
class WindowsKeyboardEmulatorVjoy extends KeyboardEmulator {
  constructor(setupProperties) {
    ...
    // if (!this.device) {
    //   throw new Error(
    //     `Could not get the device with the given ID: ${vJoyDeviceId}, this could be because the device is being controlled by another process or the device has not been created`
    //   );
    // }
    // this.buttons = this.device.buttons;
    // if (this.buttons.length < 6) {
    //   throw new Error(
    //     `You must have at least 5 buttons setup on the device, you only have ${this.buttons.length} setup on device ${vJoyDeviceId}`
    //   );
    // }
  }
...
export const getCurrentKeyboardEmulator = () => {
  return KeyboardLoggingEmulator;
