...
export class GridOutput extends AbstractOutput {
  constructor() {
    ...
    if(os.platform() !==  'win32') {
      throw new Error('You are somehow running this code on a non windows machine. That shouldnt happen. Just gonna bail now because I am about to load a bunch of windows DLLs which will break on any other machine');
    }
  ...
  async blink() {
    if(!this.windowMessage) {
      this.windowMessage = this.user32.RegisterWindowMessageA('Sensory_SwitchInput')
    }
    
    // Send a press message then a release message
    const pressResult = await this.user32.SendNotifyMessageA(65535, this.windowMessage, 1, 1);
    const releaseResult = await this.user32.SendNotifyMessageA(65535, this.windowMessage, 1, 1);

    console.log("GRID OUTPUT", {windowMessage: this.windowMessage, pressResult, releaseResult});
  }
